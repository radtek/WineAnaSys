...
BOOL CAnalasisDlg::OnInitDialog()
{
	...
	m_ListFinishedProject.InsertColumn(4,L"溯源结果",LVCFMT_LEFT,100);
	...
	m_ListDetailResult.InsertColumn(2,L"溯源结果",LVCFMT_LEFT,200);
	...
	m_ListInput.SetHeaderBackColor(168,193,221,2);           //设置扩展风格
	m_ListInput.SetRowHeigt(28);              //设置扩展风格
	m_ListInput.SetHeaderHeight(1);           //设置扩展风格

	m_ListFinishedProject.SetHeaderBackColor(168,193,221,2);           //设置扩展风格
	m_ListFinishedProject.SetRowHeigt(28);              //设置扩展风格
	m_ListFinishedProject.SetHeaderHeight(1);           //设置扩展风格

	m_ListDetailResult.SetHeaderBackColor(168,193,221,2);           //设置扩展风格
	m_ListDetailResult.SetRowHeigt(28);              //设置扩展风格
	m_ListDetailResult.SetHeaderHeight(1);           //设置扩展风格
...
Release:
	...
	if(bSendSuccess)
	{
		MessageBox(pCAnalasisDlg->m_hWnd,L"项目发送成功！请耐心等待溯源结果",L"发送成功",MB_ICONINFORMATION);
	}
	else
	{
...
void CAnalasisDlg::UpdateFinishedDB()
{
	...
	if(sqlite3_open("User\\FinishedProjectInfo.db3", &pFinishedProjectInfodb) != 0)
	{
		MessageBox(L"更新完成项目数据失败",L"更新失败",MB_ICONERROR|MB_OK);
		return;
	}
	//创建用户信息表格，已存在则创建失败，错误信息保存在szErrmsg中
	char sqlcmd[500] = "CREATE TABLE FinishedProjectInfo(\
...
afx_msg LRESULT CAnalasisDlg::OnMyPrint(WPARAM wParam, LPARAM lParam)
{
	...
	TCHAR *pszTitle[3]={L"项目名称",L"光谱文件名",L"溯源结果"};
