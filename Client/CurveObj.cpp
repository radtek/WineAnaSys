...
//CFloatPoint CCurveObj::m_MaxPosAllCurve(-10000000000.0,-10000000000.0);   //所有曲线当前最大点
//CFloatPoint CCurveObj::m_MinPosAllCurve(10000000000.0,10000000000.0);   //所有曲线当前最小点
//CFloatPoint CCurveObj::m_MaxPosAllCurveConst(-10000000000.0,-10000000000.0);      ////所有曲线极值常量，用于复位所有曲线用
//CFloatPoint CCurveObj::m_MinPosAllCurveConst(10000000000.0,10000000000.0);
//CFloatPoint CCurveObj::m_ZoomOffsetAllCurve(0.0,0.0);
...
CCurveObj::CCurveObj(CRect rect)
{
	...
	//if(m_cPlotRect.Height() > m_cPlotRect.Width())
	//{
	//	regionPoints = m_cPlotRect.Height()/m_iRegionSampleCycle + 1;  //此处默认2个像素采集一个点
	//}
	//else
	//{
    //    regionPoints = m_cPlotRect.Width()/m_iRegionSampleCycle + 1;  //此处默认2个像素采集一个点
	//}
	//m_pRegionPoints = new POINT[regionPoints*2];
...
CCurveObj::~CCurveObj()
{
	 // delete []m_pRegionPoints;
	  //m_pRegionPoints = NULL;
...
/*void CCurveObj::SetAllCureveSameBase(bool sameBase)
{
	m_bAllCurveSameBase = sameBase;
	this->ResetCurve(); 
}*/
void CCurveObj::AppendNewPoint(CFloatPoint& point)
{
	...
	//同坐标系绘图用
	/*this->CalculateMaxPoint(m_MaxPosAllCurve,&point); //计算所有曲线的极值，用于所有的曲线在相同的坐标系下绘图
	this->CalculateMinPoint(m_MinPosAllCurve,&point);
	//m_MaxPosAllCurveConst = m_MaxPosAllCurve;      
	//m_MinPosAllCurveConst = m_MinPosAllCurve;
	m_ZoomOffsetAllCurve = m_MaxPosAllCurve - m_MinPosAllCurve;  //计算所有曲线的缩放比*/
...
void CCurveObj::AppendNewPoint(CFloatPoint& point,BOOL TransAbs)
{
	...
	//也就是说，曲线默认初始显示为透过率，要以吸光度显示，必须调用ChangeToAbsorbance()，同样转回透过率必须调用ChangeToTransmittance();
...
void CCurveObj::DrawPoint(CDC *pDC,CRect plotRect,size_t _pointType)
{
	...
	m_dHorizontalFactor =plotRect.Width()/(m_ZoomOffsetFp.m_PosX+0.000000001);
	m_dVerticalFactor =plotRect.Height()/(m_ZoomOffsetFp.m_PosY+0.000000001);
	//m_dHorizontalFactor =plotRect.Width()/(m_ZoomOffsetAllCurve.m_PosX+0.000000001);
	//m_dVerticalFactor =plotRect.Height()/(m_ZoomOffsetAllCurve.m_PosY+0.000000001);	
	CPen* oldPen = pDC->SelectObject(&m_CurvePen);
	double dataX = 0.0;
	double dataY = 0.0;
...
//##ModelId=48A1458602FD
/*void CCurveObj::ResetCurve()
{
    if(m_bAllCurveSameBase)
	{
		m_MaxPosFp = m_MaxPosAllCurveConst;
		m_MinPosFp = m_MinPosAllCurveConst;
	}
	else
	{
		m_MaxPosFp = m_MaxPosFpConst;
		m_MinPosFp = m_MinPosFpConst;	
	}	
	m_ZoomOffsetFp = m_MaxPosFp-m_MinPosFp;
	m_ZoomOffsetPrevious.InitPoint(0.0,0.0);
	m_MoveOffsetFp.InitPoint(0.0,0.0);
...
//##ModelId=48A145860280
/*bool CCurveObj::HitTest(CPoint point)
{
	bool rc = false;
	if(m_iRegionPointCounter>0)
	{
		CRgn rgn;
		rgn.CreatePolygonRgn(m_pRegionPoints,m_iRegionPointCounter,ALTERNATE);
		if(rgn.PtInRegion(point))
		{
//			if(m_pDc)  //绘制选中曲线后的点
//			{
//				for(int i=1;i<m_iRegionPointCounter;i++)
//				{
//                     m_pDc->SetPixel(m_pRegionPoints[i],m_CurveColor);
//				}
//			}
			rc = true;
		}
...
//	{		
//		q=a[m-1];		
//		for(k=m-1;k>=1;k--)	//计算与X之相对应的Y值	y= A(n-1)*X^(n-1) + A(n-2)*X^(n-2) +A(n-3)*X^(n-3) .......... A(0);	
//			q=q*(x[i-1]-z)+a[k-1];	
//		
//		dt=q-y[i-1];  //Q为对应X的拟合值
//		
//		if(fabs(dt)>*dt3)  //去绝对最大偏差			
//			*dt3=fabs(dt);		
//		*dt1=*dt1+dt*dt; //=拟合值与真实值间的平方和	
//		*dt2=*dt2+fabs(dt);		
//	}
...
void CCurveObj::DestroyCurve()
{
	...
	//CCurveObj::m_MaxPosAllCurve.InitPoint(-10000000000.0,-10000000000.0);   //所有曲线当前最大点
	//CCurveObj::m_MinPosAllCurve.InitPoint(10000000000.0,10000000000.0);   //所有曲线当前最小点
	//CCurveObj::m_MaxPosAllCurveConst.InitPoint(-10000000000.0,-10000000000.0);      ////所有曲线极值常量，用于复位所有曲线用
	//CCurveObj::m_MinPosAllCurveConst.InitPoint(10000000000.0,10000000000.0);
	//CCurveObj::m_ZoomOffsetAllCurve.InitPoint(0.0,0.0);
}
...
//
//int CCurveObj::GetAllCurveCurvePar(CFloatPoint& maxPos,CFloatPoint& minPos,CFloatPoint& moveOffset,CFloatPoint& zoomOffset)
//{
	//maxPos = m_MaxPosAllCurve;
	//minPos = m_MinPosAllCurve;
	//moveOffset = m_MoveOffsetFp;
   // zoomOffset = m_ZoomOffsetAllCurve;
//	return 0;
//}
//重新计算
int CCurveObj::ReCalculateMaxMinPar()
{
	...
	for(int i=0;i < m_LinePointArray.GetCount();i++)
	{
		...
		//CalculateMaxPoint(m_MaxPosAllCurve,&_point); //计算所有曲线的极值，用于所有的曲线在相同的坐标系下绘图
		//CalculateMinPoint(m_MinPosAllCurve,&_point);
	}
	//独立坐标系绘图用
	m_MaxPosFpConst = m_MaxPosFp; //保存每条曲线的坐标变化区域
	m_MinPosFpConst = m_MinPosFp;
...
//int CCurveObj::SetAllCurveMaxMinPar(CFloatPoint MaxPoint,CFloatPoint MinPoint )
//{
	//m_MaxPosAllCurve = MaxPoint;   //所有曲线当前最大点
	//m_MinPosAllCurve = MinPoint;   //所有曲线当前最小点
	//m_MaxPosAllCurveConst = m_MaxPosAllCurve;      
	//m_MinPosAllCurveConst = m_MinPosAllCurve;
	//m_ZoomOffsetAllCurve = m_MaxPosAllCurve - m_MinPosAllCurve;  //计算所有曲线的缩放比
//	return 0;
