...
void CPCAChart::PrintChart(CDC* pDC, CPrintInfo* pInfo)//提供的打印接口
{
	...
	while(iter1!=m_CurveMap.end())
	{
		...
		if(curveObject)
		{
			curveObject->SetCurveColor(RGB(255,0,0));
			if(2 == PCANum)
				curveObject->DrawPoint(pDC,rectPlot,SQUARE);//绘制点
			else
	...
	while(iter!=m_RefCurveMap.end())
	{
		...
		iter->second->SetCurveColor(m_RefColor[*itername]);
		if(2 == PCANum)
			iter->second->DrawPoint(pDC,rectPlot,CIRCLE);//绘制点
		else
			iter->second->DrawCurve(pDC,rectPlot);//绘制曲线
...
/*CCurveObj& CPCAChart::CreateNewPCACurve(CString curveName,bool timeBaseFlag ,bool REF)
{
	CCurveObj* curveObject = NULL;

	//全体PCA
	//若为参考图谱PCA
	if(m_PCATotalMap.IsEmpty()&&(m_RefPCATotalMap.size() == 0))
	{
		CDC* pDc=GetDC();
		CSize CurveNameSize;
	...
	if(REF)
	{
		...
		if(curveObject)
		{
			...
			m_RefPCATotalMap.insert(CMultiMapToCurveObj::value_type(curveName,curveObject));
	...
	{
		//若已有曲线
		if(m_PCATotalMap.Lookup(curveName,(CObject*&)curveObject)==FALSE)
		{
			//将新曲线设为选中曲线
			curveObject = new CCurveObj(m_rectPlot);
			//this->SetCurveSelected(curveName);
			if(curveObject)
			{
				...
				m_PCATotalMap.SetAt(curveName,curveObject);
			}
			CDC* pDc=GetDC();
			CSize CurveNameSize;
			CurveNameSize = pDc->GetTextExtent(curveName);
...
CFloatPoint * CPCAChart::CreateNewPCAArray(CString curveName,size_t PCNum ,bool REF)
{
	...
	if(REF)
	{
		//if(m_RefPCATotalArray.find(curveName)==m_RefPCATotalArray.end())
		//{
		pArray = new CFloatPoint[PCNum];
		//this->SetCurveSelected(curveName);
		//
		//CMultiMapToFloatPtArray m_RefPCATotalArray;
		//CMapToFloatPtArray m_PCATotalArray;
...
void CPCAChart::SetShowParameter(int PCNum,int xPC,int yPC)
{
	...
	if(2 == PCANum)
	{
		XPC = xPC;       //x轴PC
		YPC = yPC;       //y轴PC
		//XLegand.Format(L"PC%d",XPC);//x轴标识
		//YLegand.Format(L"PC%d",YPC);//y轴标识
	}	
}
void CPCAChart::AdjustTotal2Show()
{
	...
	if(2 == PCANum)
	{
		...
		//POSITION pos = m_PCATotalMap.GetStartPosition();	
...
//删除选中曲线
/*bool CPCAChart::DeleteCurveSelected()
{
...
//##ModelId=48A145890010(CString curveName,COLORREF color/* = RGB(255,0,0)*/,int lineWidth/*=1*/)
/*void CPCAChart::SetCurveColor
{
	CCurveObj* curveObject = NULL;
	if(m_CurveMap.Lookup(curveName,(CObject*&)curveObject))
	{
...
/*void CPCAChart::SelectCurve(CString curveName)
{
	if(curveName.IsEmpty())
	{
	...
	if(m_rectPlot.PtInRect(point))
	{
			     ...
			     || m_iControlMode == CURVE_ZOOM_OUT|| m_iControlMode == CURVE_ZOOM_IN))
		{
			...
			case CURVE_ZOOM_IN:
				{
					m_recMouseSelect.BottomRight() = point;
					//					if(m_recMouseSelect.Width() > m_recMouseSelect.Height())
					//					{
					//						m_recMouseSelect.bottom = m_recMouseSelect.top + m_recMouseSelect.Width();
					//					}
					//					else
					//					{
					//						m_recMouseSelect.right = m_recMouseSelect.left + m_recMouseSelect.Height();
					//					}
					break;
				}
			}
...
int CPCAChart::ReCalculateAllCurveMaxMin()
{
	...
	if(PCANum == 2)
	{
		...
		m_ptMaxPos.m_PosY += 10;

		m_ptMinPos.m_PosX -= 10;
		m_ptMinPos.m_PosY -= 10;
	}
	...
	//m_ptZoomOffset = m_ptMaxPos - m_ptMinPos;
	//if(curveObject)
