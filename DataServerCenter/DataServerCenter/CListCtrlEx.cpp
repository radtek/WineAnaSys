	...
	for ( int nCol=0; GetColumn(nCol, &lvc); nCol++)
	{
		...
		if (nCol==0)
		{
			...
			if((GetExtendedStyle()&LVS_EX_CHECKBOXES)==LVS_EX_CHECKBOXES)
			{
				...
				bmp.LoadBitmap(IDB_BITMAP_CHECK);
				...
				CRect rcRect;
				rcRect.left = lpDrawItemStruct->rcItem.left + 3;
				rcRect.top = lpDrawItemStruct->rcItem.top + 3;
				rcRect.bottom = lpDrawItemStruct->rcItem.bottom - 3;
				rcRect.right = lpDrawItemStruct->rcItem.left + m_nRowHeight - 6;

				if(GetCheck(lpDrawItemStruct->itemID))
				{
					pDC->StretchBlt( rcRect.left, rcRect.top, rcRect.Width(), rcRect.Height(), &memDC, 13, 0, 13, 13, SRCCOPY); 
				}
				else
				{
					pDC->StretchBlt( rcRect.left, rcRect.top, rcRect.Width(), rcRect.Height(), &memDC, 0, 0, 13, 13, SRCCOPY);
				}
		...
		{
			if(nCol==0&&(GetExtendedStyle()&LVS_EX_CHECKBOXES)==LVS_EX_CHECKBOXES)
			{
				rcTemp.left+=7;
			}
...
void CListCtrlEx::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (hi.iItem >= 0)
	{
		GetItemRect(hi.iItem, rcItem, LVIR_BOUNDS );
		rcCheck.left = rcItem.left + 2;
		rcCheck.top = rcItem.top + 2;
		rcCheck.bottom = rcItem.bottom - 2;
		rcCheck.right = rcItem.left + m_nRowHeight - 4;

		if (rcCheck.PtInRect(point))
		{
			if(GetCheck(hi.iItem))
			{
				SetCheck(hi.iItem, FALSE);
			}
			else
			{
				SetCheck(hi.iItem, TRUE);
			}
