...
UINT ThreadUpdateConcentrationTraceBackModel(LPVOID lpParam)
{
	...
	if(!ConcentrationTraceBackModel.Open())
	{
		...
		pCDBManageTrueDlg->MessageBox(L"打开溯源模型库失败",L"更新模型失败",MB_ICONERROR);
		return -1;
	}
...
UINT ThreadUpdatePLSModel(LPVOID lpParam)
{
	...
	while(!AlcoholTypeIndex.IsEOF())
	{
		...
		m_db2.OpenEx(_T("DSN=白酒鉴定与溯源数据库;"),CDatabase::noOdbcDialog);
		...
		while(!rsforFlavour.IsEOF())
		{
			...
			m_db3.OpenEx(_T("DSN=白酒鉴定与溯源数据库;"),CDatabase::noOdbcDialog);
			...
			while(!rsforBrand.IsEOF())
			{
				...
				m_db4.OpenEx(_T("DSN=白酒鉴定与溯源数据库;"),CDatabase::noOdbcDialog);
...
void CDBManagePageTrue::OnBnClickedButtonComponentpredictmodelupdate()
{
	...
	CFileDialog fileDialog = CFileDialog(TRUE, L"测试", L"选择成分预测模型文件", OFN_READONLY,
	...
	if (fileDialog.DoModal() == IDOK)
	{
		...
		ComponentArray.Add(L"乙醛");
		ComponentArray.Add(L"乙酸乙酯");
		ComponentArray.Add(L"甲醇");
		ComponentArray.Add(L"丁酸乙酯");
		ComponentArray.Add(L"正丙醇");
		ComponentArray.Add(L"异丁醇");
		ComponentArray.Add(L"戊酸乙酯");
		ComponentArray.Add(L"正丁醇");
		ComponentArray.Add(L"异戊醇");
		ComponentArray.Add(L"乙酸乙酯");
		ComponentArray.Add(L"庚酸乙酯");
		ComponentArray.Add(L"乳酸乙酯");
		ComponentArray.Add(L"糠醛");
		ComponentArray.Add(L"己酸");
		ComponentArray.Add(L"β-苯乙醇");
		ComponentArray.Add(L"乳酸");
		ComponentArray.Add(L"乙酸");

		CComponentPredictModel ComponentPredictModel;

		if(!ComponentPredictModel.Open())
		{
			MessageBox(L"更新成分预测模型失败，打开数据库失败！",L"成分预测模型更新",MB_ICONERROR);
			return;
		}
		//清空数据库
		...
		MessageBox(L"成分预测模型更新成功！",L"成分预测模型更新",MB_ICONINFORMATION);
	}
}
