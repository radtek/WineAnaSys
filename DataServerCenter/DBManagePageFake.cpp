...
BEGIN_MESSAGE_MAP(CDBManagePageFake, CPropertyPage)
	...
	//ON_BN_CLICKED(IDC_BUTTON_UPDATELIBINFO_FAKE, &CDBManagePageFake::OnBnClickedButtonUpdatelibinfoFake)
...
void CDBManagePageFake::OnBnClickedButtonAddtolibFake()
{
	...
	if(IDOK == CompleteSpecAddToFakeLibInfoDlg.DoModal())
	{
		...
		CString m_strFolderPath=_currentDir;
		...
		for(int i=0;i<m_ary_PicturesPath.GetCount();i++)
		{
			//生成保存图片的文件夹
			//CString SpecID;
			//SpecID.Format(L"%s-%d",m_SpecIDTitle,i+1);
			CString newpicpath=m_strFolderPath;
			newpicpath+=L"\\";
